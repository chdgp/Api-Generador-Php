...
public static function get_dominio_now($path = false) {
     ...
     */
    public static function createStorageAndImageBase64($base64string, $idusuario, $createWebp = true, $folder = 'image' ) {
        // Verificar si el base64string contiene una coma para dividir
        if (strpos($base64string, ',') === false) return (object)["original" => "", "webp" => "", 'resp'=>'file_not_base64'];
        ...
        if ($image_info !== false && isset($image_info['mime'])) {
            ...
            if ($extension == ".jpg" || $extension == ".png" && $createWebp) {
                ...
                if ($image !== false) {
                    ...
                    $webpFilePath = $storagePathWebp . $idusuario . "_" . date('Ymd') . ".webp";
            ...
            if (file_put_contents($filePath, $fileContents) !== false) {
                ...
                return (object)["original" => $_DOMINIO.'/'.$filePath, "webp" => $_DOMINIO.'/'.$webpFilePath, "resp"=> "add_file_create" ];
            }
        }
